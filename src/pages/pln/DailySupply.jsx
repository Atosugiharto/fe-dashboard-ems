...
const DailySupply = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post(`${baseApiUrl}/chartPLNDaily`, {
        date: selectedMonth,
      });
  ...
  const series = [
    { name: "PLN Actual", type: "area", data: actualData },
    { name: "PLN Planning", type: "line", data: planningData },
  ];
  ...
  const downloadExcel = () => {
    const data = days.map((day, index) => ({
      ...
      "PLN Planning": series[1].data[index],
    }));
