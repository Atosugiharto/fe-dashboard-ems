...
const PowerQualityCard = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post(`${baseApiUrl}/cardSolarPVHourly`, {
        date: todayDate,
      });
      ...
      if (response?.data?.data) {
        ...
        setPowerData({
          ...
          "PV Prod": formatNumberForDisplayDynamic(apiData?.PVprod),
      ...
      <div className="space-y-3 4k:space-y-6 font-semibold">
        {displayData?.map((item, index) => (
          <div key={index} className="flex justify-between text-sm 4k:text-4xl">
            ...
            <span className="text-dashboard-gauge-hijau font-semibold">
              {item?.value}
            </span>
          </div>
        ))}
