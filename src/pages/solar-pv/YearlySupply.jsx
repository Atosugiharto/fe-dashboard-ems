...
const YearlySupply = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post(`${baseApiUrl}/chartSolarPVYearly`, {
  ...
  const downloadExcel = () => {
    const headers = ["Fiscal Year", "PV Actual (kWh)", "PV Planning (kWh)"];
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      `Solar_PV_Supply_${selectedYear}`
    );
    XLSX.writeFile(workbook, `Yearly_Solar_PV_Supply_${selectedYear}.xlsx`);
  };
