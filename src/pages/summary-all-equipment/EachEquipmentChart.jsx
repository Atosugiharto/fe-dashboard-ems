...
const EachEquipmentChart = ({ data = [], dateStart = "", dateEnd = "" }) => {
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(workbook, `Summary_Each_Equipment.xlsx`);
  };
  ...
  const options = {
    ...
    colors: [GlobalVariable.dashboardColor.barUngu],
    dataLabels: { enabled: false },
    xaxis: {
      categories: filterArrayText(data?.map((item) => item?.equipment)) || [],
    ...
    <div>
      <div className="flex justify-between items-center mb-1">
        <h3 className="text-white font-bold">
          Each Electrical Equipment Consumption
        </h3>
        <button
